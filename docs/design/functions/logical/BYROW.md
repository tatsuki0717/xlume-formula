...
## Description
Applies a LAMBDA to each row of an array and returns an array of results (one per row).
...
|---|---|---|---|---|
| 1 | array | range/array | Yes | The array to iterate by row. |
| 2 | lambda | range/array | Yes | A LAMBDA that receives each row and returns a scalar or array. |
...
High-level behavior: Applies a LAMBDA to each row of an array and returns an array of results (one per row).
